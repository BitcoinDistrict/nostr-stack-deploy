...
if [ -f "${CONFIGS_DIR}/blossom/config.yml.template" ]; then
  envsubst < "${CONFIGS_DIR}/blossom/config.yml.template" | sudo tee /etc/blossom/config.yml >/dev/null
else
  envsubst < "${CONFIGS_DIR}/blossom/config.yml" | sudo tee /etc/blossom/config.yml >/dev/null
fi
...
envsubst < "${CONFIGS_DIR}/nginx/blossom-http.conf.template" | sudo tee "${BLOSSOM_SITE_PATH}" >/dev/null
...
if [ -f "/etc/letsencrypt/live/${BLOSSOM_DOMAIN}/fullchain.pem" ]; then
  envsubst < "${CONFIGS_DIR}/nginx/blossom.conf.template" | sudo tee "${BLOSSOM_SITE_PATH}" >/dev/null
