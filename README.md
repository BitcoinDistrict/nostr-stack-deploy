...
- **Strfry (relay) uses `plugins/nip05_gate.py`** to accept/reject incoming events.
  - **What it does**: Supports two gate modes: `open` (accepts all events) and `nip05` (builds an allowlist from `.well-known/nostr.json` documents and accepts events whose `pubkey` is present in `names`, `verified_names`, or both).
  - **Behavior**:
  ...
  - **Key flags/env**: `STRFRY_GATE_MODE` (`open|nip05`), `NIP05_JSON_URLS`/`NIP05_JSON_URL`, `NIP05_FIELD` (`names|verified_names|both`), `ALLOW_IMPORT`, `STARTUP_GRACE_SECONDS`, `EPHEMERAL_RATE`, `EPHEMERAL_BURST`, `EPHEMERAL_MAX_BUCKETS`, `EPHEMERAL_TTL_SECONDS`.
